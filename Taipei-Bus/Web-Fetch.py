...
for bus in data:
	...
	print bus_url
	bus_info[bus] = {"name":bus,"forward":[],"backward":[]}
	...
	for bus_stop in table[2].find_all("tr"):
		...
		bus_info[bus]["forward"].append((td_data[0].string,td_data[0].a['href']))
	for bus_stop in table[3].find_all("tr"):
		td_data = bus_stop.find_all("td")
		bus_info[bus]["backward"].append((td_data[0].string,td_data[0].a['href']))
	
print json.dumps(bus_info,sort_keys=True)
